...
/* Y Combinator */
static int mls_icodegen_call_by_y_combinator( \
        ...
        struct mls_icg_customizable_built_in_procedure_list *customizable_built_in_procedure_list)
{
    ...
    /* Mark Y Combinator as been invoked */
...
static int mls_icodegen_node_form_letrec_item(struct multiple_error *err, \
        struct mls_icg_context *context, \
        struct mls_icg_fcb_block *icg_fcb_block, \
        struct mls_ast_node *ast_node_var, \
        struct mls_ast_node *ast_node_exp_body)
{
    int ret = 0;
    uint32_t id;
    uint32_t id_var;
    ...
    /* 1st, construct (\v.B) */
    if ((ret = mls_icodegen_node_form_lambda_style2(err, \
                    context, \
                    icg_fcb_block, \
                    ast_node_var, \
                    ast_node_exp_body)) != 0)
    ...
    { goto fail; }
    if ((ret = mls_icg_fcb_block_append_with_configure(icg_fcb_block, OP_PUSH, id)) != 0) { goto fail; }
    /* Make Y Combinator Lambda */
    if ((ret = mls_icodegen_call_by_y_combinator( \
                    err, \
                    context->icode, \
                    context->res_id, \
                    icg_fcb_block, \
                    context->customizable_built_in_procedure_list)) != 0)
    { goto fail; }

    /* 3rd, save to v */
    ast_node_var_atom = ast_node_var->ptr;
    if ((ret = multiply_resource_get_id( \
                    err, \
                    context->icode, \
                    context->res_id, \
    ...
    { goto fail; }
    if ((ret = mls_icg_fcb_block_append_with_configure(icg_fcb_block, OP_POPCL, id_var)) != 0) { goto fail; }
...
int mls_icodegen_node_form_letrec(struct multiple_error *err, \
        struct mls_icg_context *context, \
        struct mls_icg_fcb_block *icg_fcb_block, \
        struct mls_ast_node *ast_node)
{
    ...
    struct mls_ast_node *ast_node_bind_exp_body;
    ...
    {
        multiple_error_update(err, -MULTIPLE_ERR_ICODEGEN, "%d:%d: error: letrec: bad syntax, "
                "must contains at least binding and body", \
    ...
    else if (list_node_count == 1)
    {
        ...
        {
            multiple_error_update(err, -MULTIPLE_ERR_ICODEGEN, "%d:%d: error: letrec: bad syntax, no expression in body", \
                    mls_ast_node_ln(context->ast_node_parent), mls_ast_node_col(context->ast_node_parent));
            ret = -MULTIPLE_ERR_ICODEGEN;
            goto fail;
        }
        ...
        {
            multiple_error_update(err, -MULTIPLE_ERR_ICODEGEN, "%d:%d: error: letrec: bad syntax, no binding", \
                    mls_ast_node_ln(context->ast_node_parent), mls_ast_node_col(context->ast_node_parent));
            ret = -MULTIPLE_ERR_ICODEGEN;
            goto fail;
        }
    ...
    ast_node_bind = ast_node;
    ast_node_body = mls_icg_cdr(ast_node);

    /* Binds */
    ast_node_bind_branch_cur = mls_icg_car(ast_node_bind);
    while (ast_node_bind_branch_cur != NULL)
    {
        ...
        ast_node_bind_exp_body = mls_icg_cdr(ast_node_bind_branch_cur_in);

        if (mls_icg_is_node_atom_id(mls_icg_car(ast_node_bind_branch_cur_in)) == 0)
        {
            multiple_error_update(err, -MULTIPLE_ERR_ICODEGEN, "%d:%d: error: letrec: bad syntax, "
                    "not an identifier and expression for a binding", \
                    mls_ast_node_ln(ast_node_bind_branch_cur), mls_ast_node_col(ast_node_bind_branch_cur));
        ...
        if ((ret = mls_icodegen_node_form_letrec_item(err, \
                        context, \
                        new_icg_fcb_block, \
                        ast_node_bind_var, \
                        ast_node_bind_exp_body)) != 0)
        { goto fail; }
