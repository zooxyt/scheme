...
struct mls_icodegen_node_proc_stdnum_item
{
    const int type; 
...
static struct mls_icodegen_node_proc_stdnum_item mls_icodegen_node_proc_stdnum_items[] = 
{
    { MLS_ICG_PROC_TYPE_ARITHMETIC, mls_icodegen_node_proc_stdnum_arith },
    { MLS_ICG_PROC_TYPE_INEQU, mls_icodegen_node_proc_stdnum_inque },
};
#define MLS_ICODEGEN_NODE_PROC_STDNUM_ITEM_COUNT (sizeof(mls_icodegen_node_proc_stdnum_items)/sizeof(struct mls_icodegen_node_proc_stdnum_item))
        ...
        const int procedure_idx)
{
    ...
    switch (procedure_item->op)
    {
        case MLS_ICG_PROC_OP_MANUAL:
            for (idx = 0; idx != MLS_ICODEGEN_NODE_PROC_STDNUM_ITEM_COUNT; idx++)
            {
                if (mls_icodegen_node_proc_stdnum_items[idx].type == procedure_item->type)
                {
                    matched = 1;
                    break;
                }
            ...
            if (matched != 0)
            {
                if ((mls_icodegen_node_proc_stdnum_items[idx].func(err, \
                                    context, \
                                    icg_fcb_block, \
                                    ast_node_proc, \
                                    ast_node_args, \
