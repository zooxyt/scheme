...
int mls_icodegen_node_form_let_item(struct multiple_error *err, \
        struct mls_icg_context *context, \
        struct mls_icg_fcb_block *icg_fcb_block, \
        struct mls_ast_node *ast_node_exp)
{
    int ret = 0;
...
int mls_icodegen_node_form_let(struct multiple_error *err, \
        struct mls_icg_context *context, \
        struct mls_icg_fcb_block *icg_fcb_block, \
        struct mls_ast_node *ast_node)
{
    ...
    {
        multiple_error_update(err, -MULTIPLE_ERR_ICODEGEN, "%d:%d: error: let: bad syntax, " \
    ...
    else if (list_node_count == 1)
    {
        ...
        {
            multiple_error_update(err, -MULTIPLE_ERR_ICODEGEN, "%d:%d: error: let: bad syntax, no binding", \
                    mls_ast_node_ln(context->ast_node_parent), mls_ast_node_col(context->ast_node_parent));
            ret = -MULTIPLE_ERR_ICODEGEN;
            goto fail;
        }
    ...
    /* Retrieving arguments in the lambda */ 
    ...
    while (ast_node_bind_branch_cur != NULL)
    {
        ...
        { goto fail; }
        if ((ret = mls_icg_fcb_block_append_with_configure(new_icg_fcb_block, OP_ARGCL, id_var)) != 0) { goto fail; }
    ...
    /* Arguments count */
    if ((ret = multiply_resource_get_int(err, context->icode, context->res_id, &id_bind_count, list_node_bind_count)) != 0)
    { goto fail; }
    /* Reverse pushed arguments */
    if (list_node_bind_count > 1)
    {
        /* Arguments count for reverse */
        if ((ret = mls_icg_fcb_block_append_with_configure(icg_fcb_block, OP_PUSH, id_bind_count)) != 0) { goto fail; }
        /* Reverse */
