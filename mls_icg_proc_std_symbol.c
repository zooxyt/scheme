...
int mls_icodegen_node_proc_std_symbols(struct multiple_error *err, \
        ...
        const int procedure_idx)
{
    ...
    switch (procedure_item->value)
    {
        ...
        case MLS_ICG_PROC_TYPE_SYMBOLS:
            ...
            ret = virtual_machine_object_type_name_to_id(&type_id, "symbol", 6);
            if (ret != 0) 
            {
                multiple_error_update(err, -MULTIPLE_ERR_ICODEGEN, "\'symbol\' isn't a valid type name");
